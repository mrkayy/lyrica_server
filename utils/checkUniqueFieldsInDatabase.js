...
const checkUniqueFieldsInDatabase = (db) => async (fieldsToCheck, data, operation, filter = {}) => {
  ...
  case 'REGISTER':
    ...
    if (data && data['phone']){
      let loginFieldFilterForPhone = { $or:[] };
      loginFieldFilterForPhone['$or'].push(
        { 'phone':data['phone'] },
        { 'email':data['phone'] },
      );
      loginFieldFilterForPhone.isActive = true;
      loginFieldFilterForPhone.isDeleted = false;
      let found = await db.findOne(loginFieldFilterForPhone);
      if (found){
        return {
          isDuplicate : true,
